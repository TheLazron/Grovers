...
//creating all the elements present in currentUserBasket in the Basket
    currentUserBasket.forEach(item=>{
        additemToBasketDOM(`shop/res/productImages/${item.imageSrc}.png`, item.productTitle, item.price, item.quantity)
        console.log("paths:"+`shop/res/productImages${item.imageSrc}.png`);
    })
...
//Barba JS Transition to Shop package

// barba.init({

//     views:[     //array of pages btw which transition is going to take place
//         {
//             namespace: "home",
...
//             leave({current, next}){
//                 let done = this.async();
//                 const pageTransitionTimeline = gsap.timeline({defaults:{ease: "power2.inOut"}});
//                 pageTransitionTimeline.fromTo(current.container, 1 ,{opacity:0}, {opacity: 1});
//                 pageTransitionTimeline.fromTo(".transitionDiv", 0.75, {x:"-100%"}, {x: "0%", onComplete: done});
//                 },
//                 enter({current,next}){
//                     //scroll to the top
//                     window.scrollTo(0,0);
//                     let done = this.async();
//                     const pageTransitionTimeline = gsap.timeline({defaults:{ease: 'power2.inOut'}});
//                     pageTransitionTimeline.fromTo(".transitionDiv", 0.75, {x: "0%"}, {x: "100%", onComplete: done});
            
//                     pageTransitionTimeline.fromTo(next.container,1, {opacity: 0}, {opacity: 1});
